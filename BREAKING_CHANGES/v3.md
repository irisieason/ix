...
## Remove package `@irisieason/ix-icons` from `@irisieason/ix` base library

Prior to version 3.0.0 of `@irisieason/ix-icons` the entire icon set was loaded into the client by default, which caused unnecessary usage of bandwidth and memory.
Starting with version 3.0.0, icons can now be loaded individually, providing significant performance optimizations.
To enable individual icon loading, the @irisieason/ix-icons package now requires manual bootstrapping.
Please refer to the [`@irisieason/ix-icons`](https://github.com/irisieason/ix-icons) repository for detailed installation instructions.

For usage with one of the framework wrappers (`@irisieason/ix-angular`, `@irisieason/ix-react` and `@irisieason/ix-vue`) read the respective section below.

### Usage of icons inside `@irisieason/ix-react` and `@irisieason/ix-vue` changed
...
Create a copy task to ensure that all icons from the `@irisieason/ix-icons` package are available as a static resource. Below is an example of a vite configuration (build only) example:
...
export default defineConfig({
  plugins: [
    ...
    copy({
      ...
      targets: [
        {
          src: 'node_modules/@irisieason/ix-icons/svg/*.svg',
          dest: 'public/svg',
        },
      ],
    }),
...
### Usage of icons inside `@irisieason/ix-angular` changed
...
"assets": [
  ...
  {
  "glob": "**/*.svg",
  "input": "node_modules/@irisieason/ix-icons/svg",
  "output": "./svg"
  }
],
```
...
```ts
import { setAssetPath } from '@irisieason/ix-icons/components';
...
Of course you can still use Bootstrap together with @irisieason/ix, but you need to add it to you project manually.
...
### Utility function `convertThemeName` war removed from `@irisieason/ix-echarts`
...
```javascript
import { convertThemeName, registerTheme } from '@irisieason/ix-echarts';
