...
`position` property is treated as an internal property and not exposed in the public API anymore.
...
This breaking change only affects the function `modal` which is exported from `@irisieason/ix` (not `@irisieason/ix-angular` etc).
...
FlipTileState was changed from```
